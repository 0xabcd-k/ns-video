        ...
        if(params.drama){
            ...
            setTimeout(async ()=>{
                if(dramaResp.data.watch_no){
                    await play(dramaResp.data.watch_no,dramaResp.data)
                }else{
                    await play(1,dramaResp.data)
                }
            },1000)
        }
        setLoading(false)
    ...
    return <>
        ...
        <div className='h-main' style={{maxWidth: '500px'}}>
            {purchase != null && <>
                ...
                <div className='h-recharge-modal-box' style={{maxWidth: '450px'}}>
                    ...
                    <div className='h-recharge-modal'>
                        ...
                        <div className='h-recharge-pay-btn' onClick={async () => {
                            Occur(Event.PaymentPayerMax)
                            if (drama?.purchase) {
                                Toast.info(getText(Text.Purchased))
                            } else {
                            ...
                            <div className='finger'>
                                <img className='f-light' src={require("@/assets/finger-light.png")} alt='light'/>
                        ...
                        {gift?<>
                            ...
                            {drama?.no_redeem ? <>
                                <div className='h-recharge-no-redeem'>{getText(Text.NoRedeem)}</div>
                            </> : <>
                                <div className='h-recharge-gift-btn' onClick={()=>{
                                    setGift(true)
                                }}>
                                    <div className='h-recharge-modal-icon'>
                                        <svg t="1756372027172" className="main" viewBox="0 0 1024 1024" version="1.1"
                                             xmlns="http://www.w3.org/2000/svg" p-id="5382" width="200" height="200">
                                            <path
                                                d="M192 597.333h298.667v213.334H192V597.333z m640 0v213.334H533.333V597.333H832z m0-42.666H533.333V341.333H832v213.334zM490.667 341.333v213.334H192V341.333h298.667zM421.794 133.358l150.85 150.85-60.34 60.339-150.85-150.85z"
                                                p-id="5383" fill="#ffffff"></path>
                                            <path d="M452.265 284.184l150.85-150.85 60.34 60.34-150.85 150.85z" p-id="5384"
                                                  fill="#ffffff"></path>
                                        </svg>
                                    </div>
                                    <span>{getText(Text.RechargeReportTip)}</span>
                                </div>
                            </>}
                        </>}
            ...
            <div className='h-h-show'>
                ...
                {playNextDrama && <PlayNext setLoading={setLoading} drama={drama} recommendsList={recommendsList} onClose={()=>{
                    setPlayNextDrama(false)
                }}/>}
            ...
            <div className='h-h-next'>
                ...
                <div className='h-h-n-right'>
                    <div className='h-h-n-btn' onClick={async ()=>{
                        await play(playingVideoNo + 1,drama)
                    }}>
                        {getText(Text.Next)}
                    </div>
                </div>
            ...
            <div className='h-h-episode-box'>
                ...
                {Array.from({length: (drama?.video_num??0) - (drama?.pay_num??0)}).map((item, index) => {
                    if (index + (drama?.pay_num??0) + 1 === playingVideoNo) {
                        return <div className='h-h-episode-box-item playing' onClick={async () => {
                            await play(index + (drama?.pay_num??0) + 1,drama)
                        }}>
